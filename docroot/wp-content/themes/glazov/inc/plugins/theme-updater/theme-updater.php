 ...
 */
if ( ! function_exists( 'glazov_vt_get_current_theme_update' ) ) {
  function glazov_vt_get_current_theme_update( $updates ) {

    if ( isset( $updates->checked ) ) {

      if ( ! class_exists( 'Envato_Protected_API' ) ) {
        require_once( GLAZOV_FRAMEWORK .'/plugins/theme-updater/class-envato-protected-api.php' );
      }
  ...
  add_filter( 'pre_set_site_transient_update_themes', 'glazov_vt_get_current_theme_update' );
}

if ( ! function_exists( 'glazov_vt_admin_head' ) ) {
  function glazov_vt_admin_head() {
  ...
  add_action( 'admin_head', 'glazov_vt_admin_head' );
}
